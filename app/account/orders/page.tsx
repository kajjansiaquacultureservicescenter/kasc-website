...
export default async function AccountOrdersPage() {
  ...
  return (
    <div>
      <h2 className="font-bold text-[#0c4a6e] font-display text-lg mb-5">My Orders</h2>
      ...
      {!orders || orders.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center shadow-sm">
          ...
          <Link href="/shop" className="inline-block px-5 py-2.5 rounded-xl bg-[#0284c7] text-white text-sm font-semibold hover:bg-[#0369a1] transition-all">
            Shop Now
          </Link>
        </div>
      ) : (
          ...
          {orders.map((order: any) => (
            <div key={order.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <div className="font-mono font-bold text-[#0c4a6e] text-sm">{order.order_number}</div>
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                <div className="text-right">
                  <div className="font-bold text-[#0c4a6e]">UGX {Number(order.total_amount).toLocaleString()}</div>

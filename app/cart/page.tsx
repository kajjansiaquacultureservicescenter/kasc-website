    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-[#f8fafc]">
        <div className="bg-white rounded-3xl shadow-[var(--shadow-card)] border border-gray-100 p-12 max-w-md w-full text-center mx-4">
          ...
          <h1 className="text-2xl font-bold text-[#071e2e] mb-3 font-display">Order Received!</h1>
          ...
          <div className="space-y-3">
            ...
            <Link href="/" className="w-full flex items-center justify-center text-sm text-gray-400 hover:text-[#0f5070] transition-colors py-2">
              Back to Home
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center bg-[#f8fafc]">
        <div className="bg-white rounded-3xl shadow-[var(--shadow-card)] border border-gray-100 p-12 max-w-sm w-full text-center mx-4">
          ...
          <h1 className="text-xl font-bold text-[#071e2e] mb-2">Your cart is empty</h1>
    ...
    <div className="min-h-screen bg-[#f8fafc] py-8">
      <div className="container-wide max-w-5xl">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span>/</span>
          <span className="text-[#0f5070] font-medium">Cart & Order</span>
        </div>
              ...
              <div key={s} className="flex items-center flex-1">
                <div className="flex items-center gap-2 flex-1 justify-center">
                  <div className={cn(
                    ...
                    done ? "bg-[#226640] text-white" : active ? "bg-[#0f5070] text-white" : "bg-gray-100 text-gray-400"
                  )}>
                    {done ? <CheckCircle2 size={16} /> : i + 1}
                  </div>
                  <span className={cn("text-sm font-medium hidden sm:block", active ? "text-[#0f5070]" : done ? "text-[#226640]" : "text-gray-400")}>
                    {labels[s]}
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {step === "cart" && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-[var(--shadow-card)] overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                  <h2 className="font-bold text-[#071e2e] font-display flex items-center gap-2">
                    <ShoppingCart size={18} className="text-[#0f5070]" />
                ...
                <div className="divide-y divide-gray-50">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-4 p-5 hover:bg-gray-50/50 transition-colors group">
                      <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-gradient-to-br from-[#eef8fd] to-[#f0fcf4] shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link href={`/shop/${item.slug}`} className="font-semibold text-[#071e2e] hover:text-[#0f5070] transition-colors text-sm line-clamp-2 leading-snug block mb-1 font-display">
                          {item.name}
                        </Link>
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                          ...
                          <div className="flex items-center gap-0 bg-[#f8fafc] border border-gray-200 rounded-xl overflow-hidden">
                            <button onClick={() => updateQty(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-[#0f5070] transition-colors">
                              <Minus size={12} />
                            </button>
                            <span className="w-10 text-center text-sm font-bold text-[#071e2e]">{item.quantity}</span>
                            <button onClick={() => updateQty(item.id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-100 hover:text-[#0f5070] transition-colors">
                              <Plus size={12} />
                            </button>
                          </div>
                          ...
                          <span className="ml-auto font-bold text-[#0f5070] text-base">
                ...
                <div className="px-6 py-4 bg-[#f8fafc] border-t border-gray-100 flex justify-between items-center">
                  <Link href="/shop" className="text-sm text-gray-500 hover:text-[#0f5070] flex items-center gap-1 transition-colors">
            ...
            {step === "details" && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-[var(--shadow-card)] overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-50">
                  <h2 className="font-bold text-[#071e2e] font-display">Your Contact & Delivery Details</h2>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select value={form.region} onChange={e => updateForm("region", e.target.value)}
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#2d8ab8] bg-white transition-all">
                  ...
                  <div>
                    ...
                    <input value={form.address} onChange={e => updateForm("address", e.target.value)} placeholder="e.g. Masaka Road, 2km past Mpigi town"
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#2d8ab8] transition-all" />
                  </div>
                  <div>
                    ...
                    <textarea value={form.notes} onChange={e => updateForm("notes", e.target.value)} rows={3}
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-[#2d8ab8] transition-all resize-none" />
                  </div>
                </div>
            ...
            {step === "confirm" && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-[var(--shadow-card)] overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-50">
                  <h2 className="font-bold text-[#071e2e] font-display">Review & Confirm Order</h2>
                </div>
                <div className="p-6 space-y-5">
                    ...
                    <div className="space-y-2">
                      {items.map(item => (
                        <div key={item.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                          <div>
                            <div className="text-sm font-medium text-[#071e2e]">{item.name}</div>
                            <div className="text-xs text-gray-400">Qty: {item.quantity} × {formatPrice(item.price)}/{item.unit}</div>
                          </div>
                          <div className="font-bold text-[#0f5070] text-sm">{formatPrice(item.price * item.quantity)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#f8fafc] rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="text-gray-500">Name</div><div className="font-medium text-[#071e2e]">{form.name}</div>
                      <div className="text-gray-500">Email</div><div className="font-medium text-[#071e2e]">{form.email}</div>
                      <div className="text-gray-500">Phone</div><div className="font-medium text-[#071e2e]">{form.phone}</div>
                      <div className="text-gray-500">Region</div><div className="font-medium text-[#071e2e]">{form.region}</div>
                      {form.address && <><div className="text-gray-500">Address</div><div className="font-medium text-[#071e2e]">{form.address}</div></>}
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-[var(--shadow-card)] p-5 sticky top-24">
              <h3 className="font-bold text-[#071e2e] mb-4 font-display">Order Summary</h3>
              <div className="space-y-2 mb-4">
                {items.map(item => (
                  <div key={item.id} className="flex justify-between text-sm">
                    <span className="text-gray-600 truncate mr-2">{item.quantity}× {item.name}</span>
                    <span className="font-medium text-[#071e2e] shrink-0">{formatPrice(item.price * item.quantity)}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-gray-100 pt-3 space-y-2">
                ...
                <div className="flex justify-between font-bold text-base pt-1 border-t border-gray-100">
                  <span className="text-[#071e2e]">Estimated Total</span>
                  <span className="text-[#0f5070]">{formatPrice(subtotal)}</span>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <a href="tel:+256700000000" className="flex items-center gap-2 text-sm font-semibold text-[#0f5070] hover:underline">
